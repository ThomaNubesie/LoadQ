...
export default function VehicleSetupScreen() {
  ...
  const fallback = getFallbackColor(type);
  ...
  return (
    <SafeAreaView style={s.container}>
      <ScrollView contentContainerStyle={s.inner} keyboardShouldPersistTaps="handled">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 18 }}>
          <View style={{ flexDirection: "row", gap: 8 }}>
            {(Object.entries(VEHICLE_TYPES) as [VehicleType, any][]).map(([key, val]) => (
              <TouchableOpacity
                key={key}
                style={[s.typeChip, type === key && { borderColor: Colors.accent, backgroundColor: Colors.accent + "15" }]}
                onPress={() => setType(key)}
                activeOpacity={0.8}
              >
        ...
        <View style={[s.imgBox, { borderColor: fallback + "40" }]}>
          {imgUrl ? (
          ...
          <View style={s.seatBadge}>
            <Text style={s.seatBadgeText}>{seats} {t.seats}</Text>
          </View>
        </View>
        ...
        <TouchableOpacity
          style={[s.btn, (!make || !model || !plate || loading) && s.btnDisabled]}
          onPress={handleNext}
          disabled={!make || !model || !plate || loading}
          activeOpacity={0.85}
        >
...
const s = StyleSheet.create({
  ...
  imgBox:       { height: 140, backgroundColor: Colors.card, borderRadius: 14, borderWidth: 1, marginBottom: 20, overflow: "hidden", position: "relative", justifyContent: "center", alignItems: "center" },
