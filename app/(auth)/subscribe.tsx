  ...
  const PLANS = [
    {
      ...
      full:  "C$83.88/yr",
      per:   t.perMonth,
      desc:  t.billedAnnually,
      badge: t.save33,
      perks: [t.signIn, "Priority queue on join", "Extended return window +2 min", "Annual tax receipt"],
      popular: true,
    },
    {
      ...
      full:  null,
      per:   t.perMonth,
      desc:  t.billedMonthly,
      badge: null,
      perks: ["Full queue access", "Seat tracking + peer confirm", "In-app chat + calls", "Push notifications"],
      popular: false,
    },
  ];
    ...
    <SafeAreaView style={s.container}>
      <ScrollView contentContainerStyle={s.inner}>
        ...
        {PLANS.map(p => (
          <TouchableOpacity
            key={p.key}
            style={[s.card, plan === p.key && (p.popular ? s.cardPopular : s.cardActive)]}
          ...
          >
            <View style={s.cardHeader}>
              ...
              <View style={{ alignItems:"flex-end" }}>
                <Text style={[s.price, p.popular && { color: Colors.blue }]}>{p.price}</Text>
        ...
        <TouchableOpacity
          style={s.btn}
          onPress={() => router.push({ pathname:"/(auth)/payment", params:{ plan } })}
          activeOpacity={0.85}
        >
...
const s = StyleSheet.create({
  ...
  cardPopular: { borderColor:Colors.blue, borderWidth:1.5, backgroundColor:Colors.blue+"08" },
  cardHeader:  { flexDirection:"row", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 },
  cardName:    { fontSize:15, fontWeight:"600", color:Colors.t2 },
  cardDesc:    { fontSize:11, color:Colors.t3, marginTop:2 },
  badge:       { backgroundColor:Colors.blue+"25", borderRadius:5, paddingHorizontal:6, paddingVertical:2 },
  badgeText:   { color:Colors.blue, fontSize:9, fontWeight:"600" },
